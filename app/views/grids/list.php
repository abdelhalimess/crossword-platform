...
<body>
    ...
    <nav>
        <ul>
            <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'registered'): ?>
                ...
                <li><a href="./create.php">Créer une grille</a></li>
                <li class="logout"><a href="../../views/auth/logout.php"><i class="fas fa-sign-out-alt"></i> Déconnexion</a>
                </li>
    ...
    <section>
        <form method="GET" action="" class="search-bar">
            ...
            <select name="order" id="order">
                ...
                <option value="DESC" <?= isset($_GET['order']) && $_GET['order'] == 'DESC' ? 'selected' : ''; ?>>Descendant
                </option>
            </select>
    ...
    <!-- Pagination -->
    <div id="pagination">
        <a href="?page=<?= max($page - 1, 1); ?>&sort_by=<?= htmlspecialchars($sortBy); ?>&order=<?= htmlspecialchars($order); ?>"
            id="prev-link" class="<?= $page == 1 ? 'disabled' : ''; ?>">Précédent</a>
        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
            <a href="?page=<?= $i; ?>&sort_by=<?= htmlspecialchars($sortBy); ?>&order=<?= htmlspecialchars($order); ?>"
                id="page-<?= $i; ?>" class="<?= $i == $page ? 'active' : ''; ?>"><?= $i; ?></a>
        <?php endfor; ?>
        <a href="?page=<?= min($page + 1, $totalPages); ?>&sort_by=<?= htmlspecialchars($sortBy); ?>&order=<?= htmlspecialchars($order); ?>"
            id="next-link" class="<?= $page == $totalPages ? 'disabled' : ''; ?>">Suivant</a>
    </div>
