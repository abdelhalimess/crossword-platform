...
function createGrid(rows, columns) {
    ...
    for (let i = 0; i < rows * columns; i++) {
        ...
        cell.addEventListener("dblclick", () => {
            ...
            if (cell.style.backgroundColor === "white") {
                ...
                blackCells = blackCells.filter(pos => !(pos.row === cellRow && pos.col === cellCol));
            }

            // Afficher les positions des cellules noires dans la console
    ...
    notice.innerHTML = "Double-cliquez sur une case pour la rendre noire ou la rétablir.";
    ...
    button.type = "button"; 
        ...
        button.addEventListener("click", () => {
            ...
            function areAllCellsFilled(cellData) {
                for (const cell of cellData) {
                    if (cell.content !== "black" && cell.content.trim() === "") {
                        alert(`La cellule à la ligne ${cell.row}, colonne ${cell.col} est vide !`);
                        return false;
                    }
                }
                return true;
            ...
            if (!Hori_Clues || !Verti_Clues) {
                console.error("Tous les définitions doivent être remplis correctement avant de soumettre.");
            ...
            // grid id will not be sent 
            const gridId = 15; 
            console.log("horiz clues here : ")
            console.log(Hori_Clues);


            console.log("verti clues here : ")
            console.log(Verti_Clues);
...
function getHorizontalCluesContents(rows) {
    ...
    for (let i = 1; i <= rows; i++) {
        const input = document.querySelector(`input[placeholder="Définition ligne ${i}"]`);
        ...
        if (input) {
            ...
            console.error(`Le champ pour la définition horizontale ${i} est introuvable.`);
...
function getVerticlCluesContents(cols) {
    ...
    for (let i = 1; i <= cols; i++) {
        const input = document.querySelector(`input[placeholder="Définition colonne ${i}"]`);        
        ...
        if (input) {
            ...
            console.error(`Le champ pour la définition verticale ${i} est introuvable.`);
            return null; // Retourne null si un champ est manquant
        }
    }
